...
CREATE TABLE `comments` (
  ...
  CONSTRAINT `1` FOREIGN KEY (`snippet_id`) REFERENCES `snippets` (`id`) ON DELETE CASCADE,
  CONSTRAINT `2` FOREIGN KEY (`author_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
...
CREATE TABLE `snippets` (
  ...
  PRIMARY KEY (`id`),
  KEY `1` (`author_id`),
  CONSTRAINT `1` FOREIGN KEY (`author_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
(18,'bogdanov','$2a$10$B/JZVskiBe/5A3A6lgbRPepXuSniTkWuERMYkARPgmPpw4Glhy7ui','admin','ok');
...
/*!50001 VIEW `snippets_with_author` AS select `snippets`.`id` AS `id`,`snippets`.`author_id` AS `author_id`,`snippets`.`title` AS `title`,`snippets`.`flowers` AS `flowers`,`snippets`.`comments` AS `comments`,`snippets`.`status` + 0 AS `status`,`users`.`nickname` AS `author_nickname` from (`snippets` join `users` on(`snippets`.`author_id` = `users`.`id`)) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `users_with_enums` AS select `users`.`id` AS `id`,`users`.`nickname` AS `nickname`,`users`.`password` AS `password`,`users`.`role` + 0 AS `role`,`users`.`status` + 0 AS `status` from `users` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-03-10 19:13:49
