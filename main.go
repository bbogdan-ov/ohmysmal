...
func main() {
	...
	http.HandleFunc("/api/ban/{id}", h.UserCacheMiddleware(h.HandleApiBan))
	http.HandleFunc("/api/unban/{id}", h.UserCacheMiddleware(h.HandleApiUnban))
	http.HandleFunc("/api/clear/{id}", h.UserCacheMiddleware(h.HandleApiClear))
